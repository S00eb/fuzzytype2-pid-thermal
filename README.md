...
## Simulation Results & Operating Logic

### 1. Irradiance Logic (LDR to Adaptive Setpoint)
*Note: The light sensor does not directly turn on the fan. It adjusts the thermal tolerance (Setpoint) proactively.*

| Irradiance (Lux) | Environmental Condition | Effect on Setpoint | Logic Rationale |
| :--- | :--- | :--- | :--- |
| **0 - 200 Lux** | Night / Dark | Target rises **(≈ 58°C)** | No solar thermal load. System tolerates higher inverter temperatures to save fan energy. |
| **300 - 600 Lux** | Cloudy / Normal | Target is **Normal (50°C)** | Standard operating condition for optimal inverter cooling. |
| **700 - 1000 Lux** | Bright Sunlight | Target drops **(≈ 42°C)** | High solar thermal load predicted. Fan starts earlier proactively before inverter overheats. |

### 2. Thermal Management Logic (LM35 to PWM & LED)
*Assumes normal daylight (Setpoint = 50°C). Fan speed and LED status are strictly driven by the Fuzzy-PID PWM output.*

| Inverter Heatsink Temp | PWM Value | Motor Voltage | Fan Speed (RPM) | LED Status | Condition |
| :---: | :---: | :---: | :--- | :---: | :--- |
| **< 50.0°C** | 0 | 0.0 V | **OFF** | 🟢 **GREEN** | Temp below target. Fan is off to conserve battery energy. |
| **60.0°C** | ± 102 | ± 1.2 V | **Slow** | 🟢 **GREEN** | Initial heat buildup. Soft start circulation to prevent heat accumulation. |
| **65.0°C** | ± 280 | ± 3.3 V | **Medium** | 🟢 **GREEN** | Heat rising. Proportional cooling to maintain temperature. |
| **75.0°C** | ± 535 | ± 6.4 V | **Fast** | 🔴 **RED** | **(PWM > 350)**. Warning threshold crossed! Fan runs fast to dissipate heavy heat. |
| **85.0°C** | ± 900 | ± 10.8 V | **Very Fast** | 🔴 **RED** | Approaching critical overheat limit. Maximum cooling effort. |
| **> 100.0°C** | 999 | 12.0 V | **MAX** (100%) | 🔴 **RED** | Extreme danger. Full 12V power applied to DC Motor. |
...
|:---|:---:|:---:|:---:|:---:|:---:|:---|
| **Before** (heavy load) | 67.0°C | 769 Lux | 44.62°C | +22.38°C | 999 | 🔴 Red |
| **Instant drop** | 29.0°C | 339 Lux | 53.22°C | −24.22°C | **0** | 🟢 Green |
| **Stabilized** | 29.0°C | 339 Lux | 53.22°C | −24.22°C | **0** | 🟢 Green |

The **Derivative (D) action** in the PID acts as a sudden brake, instantly dropping the fan from PWM 999 → 0 to prevent cooling overshoot. Because the actual temperature (29.0°C) is now well below the safe target (53.22°C), the system stabilizes at PWM 0 (Fan completely OFF) to conserve battery energy. The Red LED switches to Green immediately.
...
|:---|:---|:---|
| **Normal Phase** | t = 0–14s | Room temperature (29°C), PWM resting at 0 (Off). Moderate response at 49.9°C (PWM ≈ 500) |
| **Critical Phase** | t = 15–23s, t = 35–45s | Temperature >70°C triggers exponential PWM surge to saturation (914–999) |
| **Braking Phase** | t = 23.5s, t = 45.5s | Sudden temperature drop triggers aggressive derivative braking (PWM drops instantly to 0, preventing overshoot and saving energy) |
