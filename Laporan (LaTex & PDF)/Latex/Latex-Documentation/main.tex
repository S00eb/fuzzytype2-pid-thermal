...
% ----------------------------------------------------------
\subsubsection*{3.1 Kondisi Operasi Normal ($T < 50^\circ\text{C}$)}

\begin{figure}[H]
    \centering
    \includegraphics[width=17cm]{runtampilanall_normal.png}
    \caption{Tampilan keseluruhan simulasi pada kondisi suhu normal ($T = 33{,}9^\circ\text{C}$).}
    \label{fig:run_normal}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=13cm]{vmnormal.png}
    \caption{Virtual Terminal dan Oscilloscope pada kondisi suhu normal.}
    \label{fig:vm_normal}
\end{figure}

Pada pengujian pertama, sensor suhu LM35 menunjukkan pembacaan $T = 33{,}9^\circ\text{C}$ dan sensor cahaya LDR menunjukkan $L = 369\text{ Lux}$. Berdasarkan logika \textit{adaptive setpoint}, nilai \textit{setpoint} suhu dihitung sebagai:
\begin{equation}
    \text{adaptive\_sp} = 50 + (500 - 369) \times 0{,}02 = 52{,}62^\circ\text{C}
\end{equation}

Karena suhu aktual ($33{,}9^\circ\text{C}$) masih jauh di bawah \textit{setpoint} adaptif ($52{,}62^\circ\text{C}$), \textit{error} kendali bernilai negatif. Akibatnya, keluaran kendali dijaga tetap rendah. Nilai PWM tercatat pada 337 dengan estimasi tegangan motor $V = 4{,}0\text{V}$. Karena PWM $\leq 350$, sistem mengaktifkan LED Hijau. Pada \textit{Virtual Oscilloscope}, bentuk gelombang PWM memperlihatkan \textit{duty cycle} sekitar $33{,}7\%$, yang berarti kipas berputar pada kecepatan rendah.

% ----------------------------------------------------------
\subsubsection*{3.2 Kondisi Beban Puncak ($T > 50^\circ\text{C}$)}

\begin{figure}[H]
    \centering
    \includegraphics[width=17cm]{runtampilanall_bahaya.png}
    \caption{Tampilan keseluruhan simulasi pada kondisi suhu kritis ($T = 74{,}9^\circ\text{C}$).}
    \label{fig:run_bahaya}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=13cm]{vmbahaya.png}
    \caption{Virtual Terminal dan Oscilloscope pada kondisi suhu kritis.}
    \label{fig:vm_bahaya}
\end{figure}

Pada pengujian kedua, sensor suhu dinaikkan hingga $T = 74{,}9^\circ\text{C}$ dengan intensitas cahaya $L = 749\text{ Lux}$. Nilai \textit{setpoint} adaptif dihitung menjadi:
\begin{equation}
    \text{adaptive\_sp} = 50 + (500 - 749) \times 0{,}02 = 45{,}02^\circ\text{C}
\end{equation}

\textit{Error} kendali bernilai sangat besar dan positif ($74{,}9 - 45{,}02 = 29{,}88^\circ\text{C}$). Keluaran PID yang diperkuat oleh \textit{fuzzy gain} menghasilkan nilai PWM yang sangat tinggi, tercatat berfluktuasi antara 853 hingga 999. LED Merah aktif secara konsisten karena PWM $> 350$. Pada \textit{Virtual Oscilloscope}, gelombang PWM menunjukkan \textit{duty cycle} mendekati 100\%, memaksa kipas berputar pada kecepatan maksimum.

% ----------------------------------------------------------
\subsubsection*{3.3 Respons Transisi}
\begin{figure}[H]
    \centering
    \includegraphics[width=16cm]{sebelum.png}
    \caption{Kondisi beban tinggi sebelum transisi ($T = 67{,}0^\circ\text{C}$).}
    \label{fig:transisi_awal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=16cm]{sesudah.png}
    \caption{Respons seketika sistem setelah suhu diturunkan mendadak ($T = 29{,}0^\circ\text{C}$).}
    \label{fig:transisi_akhir}
\end{figure}
Sistem mendemonstrasikan kapabilitas pemulihan (\textit{recovery}) yang sangat responsif ketika terjadi perubahan kondisi lingkungan secara drastis. Pengujian ini merekam momen ketika operator menurunkan nilai sensor secara mendadak dari kondisi beban tinggi menuju kondisi suhu ruangan.
Pada awalnya (Gambar \ref{fig:transisi_awal}), sistem beroperasi pada suhu kritis $67{,}0^\circ\text{C}$ dengan intensitas cahaya $769\text{ Lux}$. Nilai \textit{adaptive setpoint} saat itu adalah:
\begin{equation}
    \text{adaptive\_sp} = 50 + (500 - 769) \times 0{,}02 = 44{,}62^\circ\text{C}
\end{equation}

Hal ini menghasilkan \textit{error} positif yang besar ($67{,}0 - 44{,}62 = 22{,}38^\circ\text{C}$), sehingga algoritma menghasilkan keluaran PWM yang tinggi (PWM = 850, $V = 10{,}2\text{V}$) dan LED Merah menyala.
Kemudian, suhu diturunkan seketika menjadi $29{,}0^\circ\text{C}$ dan cahaya menjadi $339\text{ Lux}$. \textit{Setpoint} sistem langsung beradaptasi menjadi:
\begin{equation}
    \text{adaptive\_sp} = 50 + (500 - 339) \times 0{,}02 = 53{,}22^\circ\text{C}
\end{equation}

Perubahan mendadak ini membalikkan nilai \textit{error} menjadi sangat negatif ($29{,}0 - 53{,}22 = -24{,}22^\circ\text{C}$). Menghadapi nilai negatif ini, komponen turunan (Derivative) dan proporsional (Proportional) dari PID merespons secara instan untuk mengerem putaran kipas. 
Seperti yang tercatat secara \textit{real-time} pada log \textit{Virtual Terminal} (Gambar \ref{fig:transisi_akhir}), nilai PWM seketika jatuh bebas dari 850 menuju angka 2 ($0{,}0\text{V}$), mematikan daya secara agresif untuk mencegah \textit{overshoot} pendinginan. Setelah \textit{error} berhasil diredam, komponen integral menstabilkan keluaran pada nilai PWM 118 ($1{,}4\text{V}$). 
Begitu nilai PWM anjlok melintasi ambang batas 350 ke bawah, indikator \textbf{LED Merah seketika mati dan tergantikan oleh LED Hijau}. Fenomena \textit{switching} dan respons pengereman agresif ini membuktikan tingginya sensitivitas serta stabilitas \textit{loop} kendali \textit{Interval Type-2 Fuzzy-PID} pada arsitektur \textit{bare-metal} STM32.
