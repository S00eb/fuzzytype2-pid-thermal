...
```mermaid
graph LR
    subgraph PROTEUS["PROTEUS VSM SIMULATION ENVIRONMENT (Software-in-the-Loop)"]
        direction LR

        subgraph SENSOR["SENSOR BLOCK (Input)"]
            LM35["Temperature Sensor<br/>(LM35 via Potentiometer)<br/>Analog 0-5V"]
            LDR["Light Sensor<br/>(LDR via Potentiometer)<br/>Analog 0-5V"]
        end

        subgraph MCU["STM32F401VE (Cortex-M4, HSI 16 MHz)"]
            direction TB
            ADC["ADC<br/>Multi-Channel<br/>(12-bit)"]
            ALGO["IT2 Fuzzy-PID<br/>Algorithm"]
            PWM_GEN["PWM Generator<br/>(TIM1 CH1, 100Hz)"]
            UART_TX["UART TX<br/>(USART2, 4800 baud)"]
            I2C_TX["Soft-I2C<br/>Transmitter"]
        end

        subgraph ACTUATOR["ACTUATOR BLOCK (Output)"]
            L298["L298 Motor<br/>Driver"]
            MOTOR["DC Cooling<br/>Fan Motor (12V)"]
        end

        subgraph DISPLAY["DISPLAY BLOCK"]
            LCD["LCD 16x2<br/>I2C PCF8574"]
        end

        subgraph MONITOR["MONITORING & INSTRUMENTATION"]
            VT["Virtual Terminal<br/>(Serial Log)"]
            OSC["Oscilloscope<br/>(PWM Waveform)"]
        end

        subgraph ANALYSIS["DATA ANALYSIS"]
            GNUPLOT["GNUPlot<br/>(2D Time-Series,<br/>Scatter, 3D Surface)"]
        end

        LM35 -->|"Analog<br/>Temperature"| ADC
        LDR -->|"Analog<br/>Irradiance"| ADC
        ADC -->|"Digital 12-bit"| ALGO
        ALGO -->|"Duty Cycle<br/>0-100%"| PWM_GEN
        PWM_GEN -->|"PWM Signal"| L298
        L298 -->|"12V Drive"| MOTOR
        PWM_GEN -->|"PWM Signal"| OSC
        ALGO -->|"Log Data"| UART_TX
        UART_TX -->|"Serial TX"| VT
        ALGO -->|"Status"| I2C_TX
        I2C_TX -->|"I2C Bus"| LCD
        VT -->|"CSV Export"| GNUPLOT
    end

    subgraph INVERTER["PV INVERTER SYSTEM"]
        PV["Solar Panel"] --> INV["PV Inverter"]
        INV --> HEAT["Heat Dissipation"]
    end

    HEAT -.->|"Heat to<br/>be managed"| LM35
    PV -.->|"Solar<br/>Irradiance"| LDR
    MOTOR -.->|"Cooling<br/>Airflow"| INV
```
...
```mermaid
flowchart TD
    A(["START"]) --> B["System Initialization<br/>Clock HSI 16 MHz, GPIO, ADC 12-bit,<br/>PWM 100 Hz, UART 4800 baud, I2C LCD"]

    B --> C["Set Initial Parameters<br/>Setpoint = 50°C<br/>Reference Light = 500 Lux<br/>Kp=4.0, Ki=0.02, Kd=1.0"]

    C --> D[/"Read Temperature (LM35, PA0)<br/>and Light (LDR, PA1)<br/>via 12-bit ADC<br/>T = raw×100/4095 (°C)<br/>L = raw×1000/4095 (Lux)"/]

    D --> E["IT2 Fuzzification<br/>Map Temperature & Light to<br/>3 fuzzy sets: LOW, MED, HIGH<br/>each with Upper & Lower MF<br/>(Footprint of Uncertainty)"]

    E --> F["Evaluate 9 Fuzzy Rules (3×3)<br/>Firing Strength = MIN(µ_temp, µ_light)<br/>for both Upper and Lower bounds"]

    F --> G["Type-Reduction (Karnik-Mendel)<br/>y_upper = Σ(w_U·g)/Σw_U<br/>y_lower = Σ(w_L·g)/Σw_L<br/>fuzzy_gain = (y_upper + y_lower)/2"]

    G --> H["Compute Adaptive Setpoint<br/>adaptive_sp = 50 + (500 − light) × 0.02"]

    H --> I["PID Computation<br/>error = temp − adaptive_sp<br/>integral += error × 0.25 (clamped ±300)<br/>U = fuzzy_gain × (Kp·e + Ki·∫e + Kd·Δe)"]

    I --> J{"Output<br/>Saturation?"}
    J -->|"U > 999"| K["Clamp: U = 999"]
    J -->|"U < 0"| L["Clamp: U = 0"]
    J -->|"0 ≤ U ≤ 999"| M["U unchanged"]

    K --> N[/"Output PWM to L298 Motor Driver<br/>(Set Cooling Fan Speed)<br/>V_motor = PWM × 12 / 999"/]
    L --> N
    M --> N

    N --> O{"PWM > 350?"}
    O -->|"Yes"| P["🔴 Red LED ON (PC15)<br/>(Heavy Thermal Load)"]
    O -->|"No"| Q["🟢 Green LED ON (PC13)<br/>(Normal Operation)"]

    P --> R[/"Transmit via UART (every 500ms)<br/>Format: T:XX.XC L:XXXLx PWM:XXX V:XX.XV"/]
    Q --> R

    R --> S[/"Display on LCD I2C 16×2<br/>Row 1: T & L values<br/>Row 2: PWM & V values"/]

    S --> T[/"Visualize PWM on Oscilloscope<br/>Verify frequency (100 Hz) & duty cycle"/]

    T --> U[/"Export CSV Data<br/>for GNUPlot Analysis"/]

    U --> V["Wait Sampling Period (Δt)<br/>Sensor: 1000ms | Fuzzy: 500ms<br/>Motor: 1000ms | UART+LCD: 500ms"]
...
1. **Liu, Y.-Q.** et al. (2026). "A versatile logic-gated fluorescence turn-on nitrogen-doped carbon dots: STM32 microcontroller intelligent platform for assay of L-Histidine and Al³⁺." *Analytica Chimica Acta*.

2. **Chatzipapas, N. V.** & Karnavas, Y. L. (2025). "Adaptive Microprocessor-Based Interval Type-2 Fuzzy Logic Controller Design for DC Micro-Motor Control Considering Hardware Limitations." *Energies*, 18(22), 5781. MDPI.

3. **Ferro, G.** et al. (2026). "An embedded accelerated decentralized optimization algorithm with application to energy communities." *Journal of Energy Storage*, 116, 116047. Elsevier.

4. **Khan, H.** et al. (2026). "Microcontroller-based numerical control DC stabilized power supply: design, implementation, and performance evaluation." *Scientific Reports*, 16, 13179. Springer Nature.

5. **Hossain, M. S.**, Bashir, M. B. A. & Akter, K. (2026). "Design a low-cost solar PV data logger and assess the application and accuracy with off-grid monocrystalline panels." *Solar Energy*, 292, 113427. Elsevier.

6. **Nkinyam, C. M.** et al. (2025). "Development of a low-cost monitoring device for solar electric (PV) system using internet of things (IoT)." *Energy Reports*, 13, 4788–4804. Elsevier.

7. **Yuvaraj, R.** et al. (2025). "Real-time fire detection and suppression system using YOLO11n and Raspberry Pi for thermal safety applications." *Robotics and Autonomous Systems*, 185, 104925. Elsevier.

8. **Yang, X.** et al. (2026). "A lightweight detector with hybrid pooling and checkerboard attention for solar panel anomalies." *Solar Energy*, 290, 113310. Elsevier.

9. **Behera, S.** & Choudhury, N. B. D. (2022). "Modelling and Simulations of Modified Slime Mould Algorithm Based on Fuzzy PID to Design an Optimal Battery Management System in Microgrid." *Energies*, 15(14), 5328. MDPI.

10. **Mezouari, M.**, Megrini, M. & Gaga, A. (2025). "Real-time voltage regulation using fuzzy logic in single-ended primary-inductor converter for electric energy systems." *Energy Reports*, 13, 4006–4023. Elsevier.

11. **Sayeh, K. F.** et al. (2025). "A Novel Fuzzy-Logic-Improved Direct Power Control Strategy for DFIG-Based Wind Turbine Through Real-Time OPAL-RT Validation." *Energies*, 18(11), 2813. MDPI.

12. **Ekuewa, O. I.**, Adejare, A. A. & Kim, J. (2024). "Intelligent scheduling algorithms for Internet of Things systems considering energy storage/consumption and network lifespan." *Journal of Energy Storage*, 86, 111155. Elsevier.

13. **Sain, D.** & Lee, T. H. (2026). "Novel Mathematical Modeling of IT2 Takagi–Sugeno Fuzzy PID Controllers with Triangular FoUs and Their Application: A MagLev Case Study." *Energies*, 19(10), 2636. MDPI.

14. **Yang, J.** et al. (2025). "Adaptive equalization method of lithium battery module based on time-varying characteristics of voltage and SOC." *Journal of Energy Storage*, 107, 114980. Elsevier.

15. **Nippatla, V. R.** & Mandava, S. (2025). "Performance analysis of permanent magnet synchronous motor based on transfer function model using PID controller tuned by Ziegler–Nichols method." *e-Prime — Advances in Electrical Engineering, Electronics and Energy*, 11, 100939. Elsevier.
